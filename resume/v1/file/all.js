...
$(function() {
	$("img").lazyload({
		placeholder: location.href.substring(0,location.href.lastIndexOf('/'))+"/file/empty.gif",
		effect: "fadeIn"
	});
	$("a:has(img)").slimbox();
});
...
$(function(){
    var content=$("<div id='toc'></div>");
    var tmp="<span style='margin-bottom:8px;display:block;text-align:center'>Toc</span><ul>",article=$("article");
    for(i=0;i<article.length;i++)
        tmp+= "<li><a href='#"+article[i].id+"' class='goto"+article[i].id+"'>"+ $(article[i]).children("h2").text() +"</a></li>\n";
    content.html(tmp+"</ul>");
    $('body').append(content);
    $("#toc a").click(function(){
        to = this.className.substring(4);
        $.scrollTo($("#"+to), 1000, {
